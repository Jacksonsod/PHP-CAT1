...
export default function CheckIns() {
  ...
  const endpoint = 'http://localhost/PHP-CAT1/hotel-reservation-system-frontend/src/backend/controllers/CheckinsController.php';
  ...
  const load = React.useCallback(() => {
    ...
    axios.get(endpoint)
      ...
      .catch(err => {
        console.error('Check-ins load error:', err?.response?.status, err?.response?.data || err);
  ...
  const doCheckin = (reservationId) => {
    ...
    axios.post(endpoint, data)
      .then(res => {
        ...
        else setError(res.data?.message || 'Check-in failed');
      })
      .catch(err => {
        console.error('Check-in error:', err?.response?.status, err?.response?.data || err);
  ...
  return (
    <Box p={3} sx={{ maxWidth: 1100, mx: 'auto', textAlign: 'center' }}>
      <Typography variant="h5" fontWeight={700} mb={2}>Today's Arrivals</Typography>
      <Card sx={{ mb: 2 }}>
        <CardContent>
            ...
            {rows.map(r => (
              <TableRow key={r.id}>
                ...
                <TableCell align="right">
                  <Button size="small" variant="contained" onClick={() => doCheckin(r.id)}>Check-in</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
