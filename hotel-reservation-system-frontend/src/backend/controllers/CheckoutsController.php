...
try {
    ...
    if ($method === 'GET') {
        // Today's departures: reservations to be checked out today
        ...
        $sql = "SELECT r.reservation_id AS id, 
                ...
                WHERE r.check_out_date = CURDATE() AND r.status IN ('checked_in','confirmed')
    ...
    if ($method === 'POST') {
        ...
        if ($action === 'checkout') {
            ...
            // Insert into check_outs and update reservation status
            $stmt = $conn->prepare("INSERT INTO check_outs (reservation_id, staff_user_id, check_out_at) VALUES (?, ?, NOW())");
            ...
            $stmt2 = $conn->prepare("UPDATE reservations SET status='checked_out' WHERE reservation_id=?");
